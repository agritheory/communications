...
from communications.api.chat.formatting import (
	build_quoted_reply,
	html_to_plain_text,
	plain_text_to_html,
)
from communications.api.chat.helpers import get_user_avatar, parse_room_id
...
def send_message(
	...
	try:
		...
		user_fullname = str(
			frappe.get_value("User", current_user_id, "full_name") or current_user_id or ""
		)
...
def _send_email(
	...
	if reply_content_for_quote and reply_sender_for_quote:
		...
		quoted_reply = build_quoted_reply(
			"", reply_content_for_quote, reply_sender_for_quote, reply_date_str
		)
...
def mark_messages_seen(room_id: str) -> MarkSeenResponse:
	...
	if is_comment_room(room_id):
		...
		# Find all unseen comments for this document
		unseen_query = (
		...
		unseen = unseen_query.run(as_dict=True)

		count = 0
		for comm in unseen:
			frappe.db.set_value("Comment", comm["name"], "seen", 1)
			count += 1
	...
	# Handle Communication rooms (Email, SMS, Phone)
	room_parts = parse_room_id(room_id)
	medium = room_parts.get("medium")
	identifier = room_parts.get("identifier")
	ref_dt = room_parts.get("reference_doctype")
