...
from communications.api.chat.types import LastMessage, Room, RoomUser, UserStatus
...
def get_user_avatar(user: str | None) -> str:
	...
	User = DocType("User")
	result = (
		frappe.qb.from_(User).select(User.user_image).where(User.name == user).limit(1).run(as_dict=True)
	)
...
def build_room_from_thread(thread: dict[str, Any], current_user_id: str) -> Room:
	...
	if contact:
		...
		room_name = (
			thread.get("_external_name") or thread.get("sender_full_name") or identifier or "Unknown"
		)
