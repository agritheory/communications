...
def get_sendmail_route_locals():
	route_recipients = getattr(frappe.local, "sendmail_route_recipients", None)
	route_subject = getattr(frappe.local, "sendmail_route_subject", None)
	route_message = getattr(frappe.local, "sendmail_route_message", None)
	route_attachments = getattr(frappe.local, "sendmail_route_attachments", None)
	route_extra = getattr(frappe.local, "sendmail_route_extra", None) or {}
	return route_recipients, route_subject, route_message, route_attachments, route_extra


def apply_sendmail_route_context(
	context,
	route_subject=None,
	route_message=None,
	route_extra=None,
...
def send_slack_or_teams_dm_notification(
	...
	apply_sendmail_route_context(context, route_subject, route_message, route_extra)

	if route_recipients:
		frappe.local.sendmail_route_recipients = normalize_route_recipients(list(route_recipients))
	if route_subject is not None:
		frappe.local.sendmail_route_subject = route_subject
	if route_message is not None:
		frappe.local.sendmail_route_message = route_message
	if route_extra:
		frappe.local.sendmail_route_extra = route_extra
	try:
...
class CommunicationsNotification(Notification):
	...
	def send(self, doc):
		...
		route_recipients, route_subject, route_message, _, route_extra = get_sendmail_route_locals()
		apply_sendmail_route_context(context, route_subject, route_message, route_extra)
		enqueued = False
