	...
	<Teleport :to="flyinMode ? '.flyout-drawer' : 'body'">
		<Transition name="panel-slide">
			<div
				...
				:class="{ 'contact-panel-overlay--flyin': flyinMode }"
				@click="$emit('close')">
				<div class="contact-panel" @click.stop>
					...
					<div class="contact-panel-body">
						...
						<div class="panel-section">
							...
							<div v-if="room?.contactName" class="info-row">
								...
								<div class="info-content">
									...
									<a :href="`/app/contact/${room.contactName}`" class="info-value link" target="_blank">
										{{ room.contactName }}
									</a>
								</div>
							</div>
						...
						<div v-if="room?.contactName" class="panel-section">
							...
							<div v-else-if="hasLinkedDocuments">
								<div v-for="group in linkedGroups" :key="group" class="linked-group">
									<h5 class="group-title">{{ group }}</h5>
									<ul class="linked-list">
										<li v-for="doc in linkedDocuments[group]" :key="`${doc.doctype}-${doc.name}`" class="linked-item">
											...
											<div class="linked-content">
												...
												<a :href="`/app/${encodeDoctype(doc.doctype)}/${doc.name}`" class="linked-link" target="_blank">
													{{ doc.title || doc.name }}
												</a>
											</div>
										</li>
					...
					<div class="contact-panel-footer">
						<button v-if="room?.contactName" class="btn btn-primary" @click="openContact">
							{{ __('Open Contact') }}
						</button>
						<button v-if="room?.emailId" class="btn btn-secondary" @click="composeEmail">
							<svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
								<path
									stroke-linecap="round"
									stroke-linejoin="round"
	...
	async contactName => {
		...
		const [linkedResponse, contactResponse] = await Promise.allSettled([
			frappe.call<LinkedDocuments>({
				method: 'communications.api.chat.links.get_linked_documents',
				args: {
					doctype: 'Contact',
					docname: contactName,
				},
