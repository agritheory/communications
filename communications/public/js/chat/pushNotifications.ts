...
const PUSH_API = 'communications.api.chat.push';
...
/**
 * Register (or retrieve) the communications service worker.
 * The SW file is served as a static asset from /assets/communications/js/chat/sw-communications.js
 *
 * Note: Service workers require HTTPS or localhost. On HTTP domains, registration will fail silently.
 */
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  if (!window.isSecureContext) {
    console.log('[communications] Service worker requires HTTPS or localhost. Push notifications disabled.');
    return null;
  }

  try {
    swRegistration = await navigator.serviceWorker.register(
      '/assets/communications/js/chat/sw-communications.js',
      { scope: '/app/' }
    );
    console.log('[communications] Service worker registered:', swRegistration.scope);
    ...
    navigator.serviceWorker.addEventListener('message', (event) => {
      if (event.data?.type === 'NAVIGATE_TO_CHAT' && event.data.room_id) {
        ...
        window.dispatchEvent(
          new CustomEvent('communications:navigate', {
    ...
    console.error('[communications] Service worker registration failed:', err);
    return null;
  }
}
...
export async function subscribeToPush(): Promise<boolean> {
  if (!isPushSupported()) {
    console.warn('[communications] Push notifications are not supported in this browser.');
    return false;
  }
  ...
  if (permission !== 'granted') {
    console.log('[communications] Notification permission denied.');
    return false;
  }
  ...
  if (!vapidKey) {
    console.error('[communications] VAPID public key not available.');
    return false;
  }
  ...
  try {
    ...
    console.log('[communications] Push subscription saved.');
    return true;
  } catch (err) {
    console.error('[communications] Push subscribe failed:', err);
    return false;
  }
}
...
export async function unsubscribeFromPush(): Promise<boolean> {
  ...
  try {
    ...
    console.log('[communications] Push unsubscribed.');
    return true;
  } catch (err) {
    console.error('[communications] Push unsubscribe failed:', err);
    return false;
  }
}
