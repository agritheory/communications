<template>
	<div class="chat-container" :class="{ 'chat-dark': isDarkMode, 'chat-container--flyin': flyinMode }">
		<!-- Filter toolbar -->
		<div class="chat-toolbar" :class="{ 'chat-toolbar--flyin': flyinMode }">
			<div class="toolbar-left">
				<button
					v-if="flyinMode"
					type="button"
					class="conversations-toggle"
					:class="{ 'conversations-toggle--active': conversationsPanelOpen }"
					...
					<span v-if="unreadCount > 0 && !conversationsPanelOpen" class="conversations-toggle__badge">{{
						unreadCount
					}}</span>
				</button>
				<h3 v-if="!flyinMode" class="toolbar-title">{{ __('Messages') }}</h3>
				<span v-if="!flyinMode && unreadCount > 0" class="unread-badge">{{ unreadCount }}</span>
			</div>
		...
		<ChatWindow
			class="chat-window"
			:class="{ 'chat-window--flyin': flyinMode }"
			...
			:rooms-list-opened="conversationsPanelOpen"
			:show-search="true"
			:show-add-room="false"
		...
		<Teleport :to="flyinMode ? '.flyout-drawer' : 'body'">
			<div
				v-if="showCommPanel"
				class="comm-panel-overlay"
				:class="{ 'comm-panel-overlay--flyin': flyinMode }"
...
export default defineComponent({
	...
	setup(props) {
		...
		const conversationsPanelOpen = ref(!props.flyinMode)

		function toggleConversationsPanel(): void {
			conversationsPanelOpen.value = !conversationsPanelOpen.value
		}

		function onToggleRoomsList(event: { opened?: boolean }): void {
			if (!props.flyinMode || event?.opened === undefined) {
				return
			}
			conversationsPanelOpen.value = event.opened
		}

		watch(currentRoomId, roomId => {
			if (props.flyinMode && roomId) {
				conversationsPanelOpen.value = false
			}
		})
		...
		async function archiveRoom(roomId: string): Promise<void> {
			...
			try {
				const response = await fetch('/api/method/communications.api.chat.api.archive_room', {
					method: 'POST',
					headers: {
		...
		async function deleteRoom(roomId: string): Promise<void> {
			...
			try {
				const response = await fetch('/api/method/communications.api.chat.api.delete_room', {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
						'X-Frappe-CSRF-Token': (window as any).frappe?.csrf_token || '',
		...
		function onMessageAction(data: { roomId: string; action: MessageAction; message: Message }): void {
			...
			switch (data.action.name) {
				...
				case 'openCommunication':
					if (data.message?.communicationName) {
						window.open(`/app/communication/${data.message.communicationName}`, '_blank')
					}
					break
			}
		}
...
.chat-container.chat-container--flyin {
	height: 100%;
	max-height: 100%;
	min-height: 0;
}
