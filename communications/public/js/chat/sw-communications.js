...
/**
 * Service Worker for communications push notifications.
 *
...
const NOTIFICATION_TAG_PREFIX = 'communications-chat-'
...
self.addEventListener('push', event => {
	...
	const options = {
		body: payload.body || '',
		icon: payload.icon || '/assets/communications/images/message-icon.png',
		badge: payload.badge || '/assets/communications/images/badge-icon.png',
		tag: payload.tag || NOTIFICATION_TAG_PREFIX + (payload.room_id || 'general'),
		renotify: true,
...
self.addEventListener('notificationclick', event => {
	...
	const targetUrl = event.notification.data?.url || '/app/communication/view/chat'
...
self.addEventListener('pushsubscriptionchange', event => {
	event.waitUntil(
		self.registration.pushManager.subscribe(event.oldSubscription.options).then(subscription => {
			...
			return fetch('/api/method/communications.api.chat.push.save_push_subscription', {
				method: 'POST',
